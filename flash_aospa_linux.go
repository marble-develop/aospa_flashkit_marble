...
func main() {
	...
	submitButton2 := widget.NewButton("Reboot Phone", func() {
		...
		if _, err := os.Stat(destDir + "/platform-tools-linux/fastboot"); err == nil || os.IsExist(err) {
			...
			cmd2, _ := exec.Command(destDir+"/platform-tools-linux/fastboot", "reboot").CombinedOutput()
	...
	submitButton3 := widget.NewButton("Flash Kernel", func() {
		...
		outputTextArea.CursorRow = outputTextArea.CursorRow + 1
		downloadFile("https://github.com/topjohnwu/Magisk/releases/download/v27.0/Magisk-v27.0.apk", filepath.Dir(ex)+"/Magisk.zip")
		outputTextArea.SetText(outputTextArea.Text + "\n" + "Extracting magiskboot...")
		outputTextArea.CursorRow = outputTextArea.CursorRow + 1
		Unzip(filepath.Dir(ex)+"/Magisk.zip", filepath.Dir(ex)+"/magiskboot")

		outputTextArea.SetText(outputTextArea.Text + "\n" + "Patching boot.img ...")
		cmd := exec.Command("chmod", "+x", filepath.Dir(ex)+"/magiskboot/lib/x86_64/libmagiskboot.so")
		cmd.Dir = filepath.Dir(ex)
		cmd.Run()

		cmd = exec.Command(filepath.Dir(ex)+"/magiskboot/lib/x86_64/libmagiskboot.so", "unpack", filepath.Dir(ex)+"/boot.img")
		cmd.Dir = filepath.Dir(ex)
		...
		cmd = exec.Command("cp", "-f", "Image", "kernel")
		cmd.Dir = filepath.Dir(ex)
		cmd.Run()
		outputTextArea.SetText(outputTextArea.Text + "\n" + "Patching kernel ...")
		cmd = exec.Command(filepath.Dir(ex)+"/magiskboot/lib/x86_64/libmagiskboot.so", "repack", filepath.Dir(ex)+"/boot.img")
		cmd.Dir = filepath.Dir(ex)
		cmd.Run()
		outputTextArea.SetText(outputTextArea.Text + "\n" + "Patching completed.")
		outputTextArea.CursorRow = outputTextArea.CursorRow + 1
		...
		outputTextArea.CursorRow = outputTextArea.CursorRow + 1
		cmd = exec.Command(destDir+"/platform-tools-linux/fastboot", "reboot", "fastboot")
		cmd.Dir = filepath.Dir(ex)
		cmd.Run()
		cmd2, _ := exec.Command(destDir+"/platform-tools-linux/fastboot", "flash", "boot", filepath.Dir(ex) + "/new-boot.img").CombinedOutput()
